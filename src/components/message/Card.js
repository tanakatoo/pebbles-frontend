  ...
  return (
    <div className='py-4 col-span-full flex items-center'>
      ...
      <div className='flex flex-col flex-grow px-4'>
        <p className={`mobile-body-2 font-PoppinsMedium ${data.read ? "" : "text-link"}`} >{username == data.from ? data.to : data.from}</p>
        <p className={data.read ? "" : "text-link"} >{message}</p>
      </div>
      <p className='text-gray mobile-label-2'>{moment(data.sent_at).format('l')}</p>
    </div >
  )
}
